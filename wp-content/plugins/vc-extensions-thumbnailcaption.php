<?php
/*
Plugin Name: Visual Composer Add-on - Thumbnail with Caption
Description: Help you to add Thumbnail with Caption in the Visual Composer, optional thumbnail carousel, caption title, caption content, caption button in the backend.
Author: Sike
Version: 1.2
...
if (!class_exists('VC_Extensions_CQThumbnailCaption')) {
  ...
  function vc_extensions_thumbnailcaption_notice(){
    $plugin_data = get_plugin_data(__FILE__);
    echo '
    <div class="updated">
      <p>'.sprintf(__('<strong>%s</strong> requires <strong><a href="http://codecanyon.net/item/visual-composer-page-builder-for-wordpress/242431?ref=sike" target="_blank">Visual Composer</a></strong> plugin to be installed and activated on your site.', 'vc_extensions_thumbnailcaption'), $plugin_data['Name']).'</p>
    </div>';
  }
  if (!defined('ABSPATH')) die('-1');


  function vc_extensions_cqthumbnailcaption_init(){
    if (!defined('WPB_VC_VERSION')) {add_action('admin_notices', 'vc_extensions_cqthumbnailcaption_notice'); return;}
    wp_register_style('vc_extensions_admin_thumbcaption', plugins_url('css/admin_icon.css', __FILE__));
    wp_enqueue_style('vc_extensions_admin_thumbcaption');
    if(!function_exists('aq_resize')) require_once('aq_resizer.php');
    if(class_exists('VC_Extensions_CQThumbnailCaption')&&!function_exists('cq_vc_cqthumbnailcaption_func')) $vc_extensions_cqthumbnailcaption = new VC_Extensions_CQThumbnailCaption();
  }

  add_action('init', 'vc_extensions_cqthumbnailcaption_init');
